...
public class OrderTransactionService {
    ...
    public OrderResponse createOrder(Long userId, CreateOrderRequest request, List<Long> lockedVariantIds) {
        Cart cart = getCartForCheckout(userId);

        validateLockedVariantIds(cart, lockedVariantIds);
    ...
    private void validateLockedVariantIds(
            Cart cart,
            List<Long> lockedVariantIds
    ) {
        List<Long> currentVariantIds = cart.getCartItems().stream()
                .map(cartItem -> cartItem.getProductVariant().getId())
                .distinct()
                .sorted()
                .toList();

        if (!currentVariantIds.equals(lockedVariantIds)) {
            throw new ConflictException(
                    "Giỏ hàng đã thay đổi, vui lòng thử lại"
            );
        }
    }
    ...
    private void restoreStock(Order order) {
        for (OrderItem orderItem : order.getOrderItems()) {
            ...
            variant.setStockQuantity(
                    variant.getStockQuantity() + orderItem.getQuantity()
            );
        }
    }
